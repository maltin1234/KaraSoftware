  ...
  computed: {
    filteredProducts() {
      return this.$store.state.assets.assets
        .filter(this.byId)
        .filter(this.bySerial)
        .filter(this.byName)
        .filter(this.byLocation)
        .filter(this.byStatus)
        .filter(this.byComptyp)
        .filter(this.byDate);
    },
  },
  methods: {
    ...
    bySerial() {
      console.log(this.searchSerial);
      if (this.searchSerial == this.$store.state.assets.assets.serialNum) {
        return this.$store.state.assets.assets.serialNum;
      } else {
        return this.$store.state.assets.assets;
      }
    },
    byName() {
      console.log("touched");
      if (this.searchName) {
        return this.$store.state.assets.assets.filter((item) => {
    ...
    byComptyp() {
      if (this.searchComptyp) {
        return this.$store.state.assets.assets.filter((item) => {
          return this.searchComptyp
            .toLowerCase()
            .split(" ")
            .every((v) => item.title.toLowerCase().includes(v));
        });
